...
class PhysicalTrainer():
    ...
    def __init__(self, training_database_file_f, training_database_file_m, testing_database_file_f,
        ...
        self.CTRL_PNL['num_GPU_dat_splits'] = 3
        ...
        self.train_xa_flat = PreprocessingLib().preprocessing_create_pressure_angle_stack(self.train_x_flat,
                                                                                self.train_a_flat,
        ...
        self.train_xa_flat = TensorPrepLib().append_input_depth_contact(np.array(self.train_xa_flat),
        ...
        self.test_xa_flat = PreprocessingLib().preprocessing_create_pressure_angle_stack(self.test_x_flat,
                                                                               self.test_a_flat,
                                                                               self.CTRL_PNL['incl_inter'], self.mat_size,
                                                                               self.CTRL_PNL['clip_sobel'],
                                                                               self.CTRL_PNL['verbose'])

        self.test_xa_flat = TensorPrepLib().append_input_depth_contact(np.array(self.test_xa_flat),
                                                              include_pmat_contact = self.CTRL_PNL['incl_pmat_cntct_input'],
                                                              mesh_depth_contact_maps_input_est = self.depth_contact_maps_input_est,
                                                              include_mesh_depth_contact_input_est = self.CTRL_PNL['depth_map_input_est'],
                                                              mesh_depth_contact_maps = self.depth_contact_maps,
    ...
    def init_convnet_train(self):
        ...
        self.train_xa_flat = np.array(self.train_xa_flat)
        self.train_y_flat = np.array(self.train_y_flat)
        train_sort_order = list(np.arange(0, len(self.train_xa_flat)).astype(int))

        self.test_xa_flat = np.array(self.test_xa_flat)
        self.test_y_flat = np.array(self.test_y_flat)
        test_sort_order = list(np.arange(0, len(self.test_xa_flat)).astype(int))




        if self.CTRL_PNL['shuffle'] == True:

            shuffle(train_sort_order)
            self.train_xa_flat = self.train_xa_flat[train_sort_order]
            self.train_y_flat = self.train_y_flat[train_sort_order]

           # shuffle(test_sort_order)
           # self.test_xa_flat = self.test_xa_flat[test_sort_order]
           # self.test_y_flat = self.test_y_flat[test_sort_order]


        print np.shape(self.train_xa_flat), 'Input training data shape'
        print np.shape(self.train_y_flat), 'Output training data shape'

        #load up all the testing data. it's probably not going to be very big so we won't split it
        self.test_x_tensor = torch.Tensor(self.test_xa_flat)
        self.test_y_tensor = torch.Tensor(self.test_y_flat)

        print np.shape(self.test_xa_flat), 'Input testing data shape'
        print np.shape(self.test_y_flat), 'Output testing data shape'

        #self.test_x_tensor = self.test_x_tensor.unsqueeze(1)
        self.test_dataset = torch.utils.data.TensorDataset(self.test_x_tensor, self.test_y_tensor)
        self.test_loader = torch.utils.data.DataLoader(self.test_dataset, self.CTRL_PNL['batch_size'], False)


        num_im_per_bigbatch = int(len(self.train_xa_flat)/self.CTRL_PNL['num_GPU_dat_splits'])
        num_im_per_bigbatch_remainder = len(self.train_xa_flat) - num_im_per_bigbatch*self.CTRL_PNL['num_GPU_dat_splits']
        #print len(self.train_xa_flat), num_im_per_bigbatch, num_im_per_bigbatch_remainder



        # train the model one epoch at a time
        for epoch in range(1, self.CTRL_PNL['num_epochs'] + 1):
            for bigbatch_idx in range(self.CTRL_PNL['num_GPU_dat_splits']):
                idx_low = bigbatch_idx*num_im_per_bigbatch
                idx_high = (bigbatch_idx+1)*num_im_per_bigbatch
                if bigbatch_idx == self.CTRL_PNL['num_GPU_dat_splits'] - 1:
                    idx_high += num_im_per_bigbatch_remainder


                self.train_x_tensor = torch.Tensor(self.train_xa_flat[idx_low:idx_high, :])
                self.train_y_tensor = torch.Tensor(self.train_y_flat[idx_low:idx_high, :])


                print "IDX LOW, IDX HIGH",  idx_low, idx_high
                ...
                self.train_loader = torch.utils.data.DataLoader(self.train_dataset, self.CTRL_PNL['batch_size'], False)

...
import convnet_legacy as convnet
...
class PhysicalTrainer():
    ...
    def __init__(self, training_database_file_f, training_database_file_m, testing_database_file_f, testing_database_file_m, opt):
        ...
        self.save_name = '_' + opt.losstype+'_synthreal_s'+str(TEST_SUBJECT)+'_3xreal_' + str(self.batch_size) + 'b_' + str(self.num_epochs) + 'e'
        ...
        if len(self.train_x_flat_real) != 0:
            ...
            print np.shape(self.train_x_flat), np.shape(self.train_x_flat)
    ...
    def init_convnet_train(self):
        ...
        if self.loss_vector_type == 'direct':
            ...
            try:
                self.model = torch.load('/home/ubuntu/data/synth/convnet'+self.save_name+'_200e.pt')
                self.save_amount = '300'
            except:
                try:
                    self.model = torch.load('/home/ubuntu/data/synth/convnet'+self.save_name+'_100e.pt')
                    self.save_amount = '200'
                except:
                    self.model = convnet.CNN(self.mat_size, fc_output_size, hidden_dim, kernel_size, self.loss_vector_type, self.batch_size, filepath=filepath_prefix)
        ...
        if self.loss_vector_type == None:
            self.optimizer2 = optim.Adam(self.model.parameters(), lr=0.000002, weight_decay=0.0005)
            ...
            self.optimizer2 = optim.Adam(self.model.parameters(), lr=0.000002, weight_decay=0.0005)  #0.000002 does not converge even after 100 epochs on subjects 2-8 kin cons. use .00001
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if self.opt.aws == True:
                if epoch == 100: torch.save(self.model, '/home/ubuntu/data/synth/convnet'+self.save_name+'_'+self.save_amount+'e.pt')
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if self.loss_vector_type == 'direct':
                    ...
                    images_up_non_tensor = PreprocessingLib().preprocessing_add_image_noise(np.array(PreprocessingLib().preprocessing_pressure_map_upsample(batch[0].numpy()[:, :, :, :], multiple = 2)), 0)
                    ...
                    batch[0], batch[1], _, _ = SyntheticLib().synthetic_master(batch[0], batch[1], batch[6],
                                                                            num_images_manip = 2,
                                                                            flip=True, shift=True, scale=False,
                                                                            include_inter=self.include_inter,
                                                                            loss_vector_type=self.loss_vector_type)
                    ...
                    print loss_betas, loss_eucl
                    loss = loss_eucl + loss_betas
    ...
    def validate_convnet(self, verbose=False, n_batches=None):
        ...
        for batch_i, batch in enumerate(self.test_loader):
            ...
            if self.loss_vector_type == 'direct':
                ...
                batch.append(batch[1][:, 32:33])  # mass, kg
                batch.append(batch[1][:, 33:34])  # height, kg
...
if __name__ == "__main__":
    ...
    if opt.quick_test == True:
        ...
        if network_design == True:
            ...
            female_subjects = [12, 16, 17, 18]
            for subject in female_subjects:
                if TEST_SUBJECT == subject:
                    print "skipping", TEST_SUBJECT
                else:
                    training_database_file_f.append(filepath_prefix_qt+'data/real/subject_'+str(subject)+'/p_files/trainval_200rlh1_115rlh2_75rlh3_175rllair.p')
                    training_database_file_f.append(filepath_prefix_qt+'data/real/subject_'+str(subject)+'/p_files/trainval_sit175rlh_sit120rll.p')  
            ...
            male_subjects = [9, 10, 11, 13, 14, 15]
            for subject in male_subjects:
                if TEST_SUBJECT == subject:
                    print "skipping", TEST_SUBJECT
                else:
                    training_database_file_m.append(filepath_prefix_qt+'data/real/subject_'+str(subject)+'/p_files/trainval_200rlh1_115rlh2_75rlh3_175rllair.p')
                    training_database_file_m.append(filepath_prefix_qt+'data/real/subject_'+str(subject)+'/p_files/trainval_sit175rlh_sit120rll.p')  



            test_database_file_m.append(filepath_prefix_qt+'data/real/subject_'+str(TEST_SUBJECT)+'/p_files/trainval_200rlh1_115rlh2_75rlh3_175rllair.p') 
            test_database_file_m.append(filepath_prefix_qt+'data/real/subject_'+str(TEST_SUBJECT)+'/p_files/trainval_sit175rlh_sit120rll.p')   

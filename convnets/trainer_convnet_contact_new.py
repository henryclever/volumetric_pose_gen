...
import convnet_contact_new as convnet
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if self.loss_vector_type == 'direct':
                    ...
                    synth_real_switch = Variable(batch[2].type(dtype).to(self.device), requires_grad = True)
                    ...
                    images_up = Variable(torch.Tensor(images_up_non_tensor).type(dtype).to(self.device), requires_grad=False)
                    images, targets = Variable(batch[0].type(dtype).to(self.device), requires_grad = False), Variable(batch[1].type(dtype).to(self.device), requires_grad = False)

                    scores_zeros = np.zeros((batch[0].numpy().shape[0], 24))  # 24 is joint euclidean errors
                    scores_zeros = Variable(torch.Tensor(scores_zeros).type(dtype).to(self.device))
                    ...
                    weight_input = torch.ones((images_up.size()[0], images_up.size()[2]*images_up.size()[3])).type(dtype).to(self.device)
                    weight_input*=batch[7].type(dtype).to(self.device)
                    weight_input = weight_input.view((images_up.size()[0], 1, images_up.size()[2], images_up.size()[3]))
                    height_input = torch.ones((images_up.size()[0], images_up.size()[2]*images_up.size()[3])).type(dtype).to(self.device)
                    height_input*=batch[8].type(dtype).to(self.device)
                    ...
                    images, targets, betas = Variable(batch[0].type(dtype).to(self.device), requires_grad = False), \
                                             Variable(batch[1].type(dtype).to(self.device), requires_grad = False), \
                                             Variable(batch[2].type(dtype).to(self.device), requires_grad = False)

                    angles_gt = Variable(batch[3].type(dtype).to(self.device), requires_grad = True)
                    root_shift = Variable(batch[4].type(dtype).to(self.device), requires_grad = True)
                    gender_switch = Variable(batch[5].type(dtype).to(self.device), requires_grad = True)
                    synth_real_switch = Variable(batch[6].type(dtype).to(self.device), requires_grad = True)

                    mesh_depth_array = batch[9].type(dtype).to(self.device)
                    mesh_contact_array = batch[10].type(dtype).to(self.device)

                    self.optimizer.zero_grad()
                    ground_truth = np.zeros((batch[0].numpy().shape[0], 82)) #82 is 10 shape params and 72 joint locations x,y,z
                    ground_truth = Variable(torch.Tensor(ground_truth).type(dtype).to(self.device))
    ...
    def validate_convnet(self, verbose=False, n_batches=None):
        ...
        for batch_i, batch in enumerate(self.test_loader):
            ...
            if self.loss_vector_type == 'direct':
                ...
                scores_zeros = Variable(torch.Tensor(np.zeros((batch[1].shape[0], 10))).type(dtype).to(self.device), requires_grad=False)
                ...
                gender_switch = Variable(batch[2].type(dtype).to(self.device), requires_grad=False)

                weight_input = torch.ones((images_up.size()[0], images_up.size()[2] * images_up.size()[3])).type(dtype).to(self.device)
                weight_input *= batch[3].type(dtype).to(self.device)
                weight_input = weight_input.view((images_up.size()[0], 1, images_up.size()[2], images_up.size()[3]))
                height_input = torch.ones((images_up.size()[0], images_up.size()[2] * images_up.size()[3])).type(dtype).to(self.device)
                height_input *= batch[4].type(dtype).to(self.device)
                height_input = height_input.view((images_up.size()[0], 1, images_up.size()[2], images_up.size()[3]))
                images_up = torch.cat((images_up, weight_input, height_input), 1)
                ...
                scores_zeros = Variable(torch.Tensor(np.zeros((batch[0].numpy().shape[0], 10))).type(dtype).to(self.device))

...
class PhysicalTrainer():
    ...
    def __init__(self, training_database_file_f, training_database_file_m, testing_database_file_f, testing_database_file_m, opt):
        ...
        print 'saving name is: '+self.save_name
        ...
        self.train_x_flat = []  # Initialize the training synth pressure mat list
        ...
        train_xa = TensorPrepLib().append_input_depth_contact(np.array(train_xa),
                                                              ...
                                                              include_mesh_depth_contact = True,
                                                              include_pmat_contact = True)
        self.train_x_tensor = torch.Tensor(train_xa)


        self.train_y_flat = []  # Initialize the training output list
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if self.loss_vector_type == 'direct':
                    ...
                    #print loss_eucl.data.numpy(), loss_betas.data.numpy(), loss_mesh_depth.data.numpy(), loss_mesh_contact.data.numpy(), "LOSSES"

                    if self.opt.reg_angles == True:
                        loss_angs = self.criterion(scores[:, 34:106], scores_zeros[:, 34:106])
                        loss = loss_betas + loss_eucl + loss_angs + loss_mesh_contact + loss_mesh_depth
                    else:
